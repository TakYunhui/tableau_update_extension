...
function renderContext(context) {
  const entries = [
    ["purposeItem", "purposeText", context.purpose],
    ["usageItem", "usageText", context.usage],
    ["collaborationItem", "collaborationText", context.collaboration]
  ];

  let visibleCount = 0;
  for (const [itemId, textId, value] of entries) {
    const visible = Boolean(value);
    setSectionVisible(itemId, visible);
    if (visible) {
  ...
  setSectionVisible("contextSection", visibleCount > 0);
}
